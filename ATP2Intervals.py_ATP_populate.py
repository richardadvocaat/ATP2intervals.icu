...
def get_athlete_name(athlete_id, username, api_key):
    with requests.Session() as session:
        response = session.get(url_profile, auth=HTTPBasicAuth(username, api_key), headers=API_headers)
        ...
        if response.status_code == 200:
            ...
            return full_name.split()[0] if full_name else 'Athlete'
        else:
...
def get_wellness_data(athlete_id, username, api_key, oldest_date, newest_date):
    ...
    with requests.Session() as session:
        response = session.get(url_wellness, headers=API_headers, auth=HTTPBasicAuth(username, api_key))
        if response.status_code == 200:
            data = response.json()
...
def calculate_weekly_loads(wellness_data):
    ...
    for entry in wellness_data:
        ...
        year_week = f"{date.isocalendar()[0]}-{date.isocalendar()[1]}"
...
def get_weekly_loads(athlete_id, username, api_key, oldest_date, newest_date):
    ...
    return calculate_weekly_loads(wellness_data)
...
def delete_events_with_prefix(athlete_id, username, api_key, oldest_date, newest_date, category, name_prefix):
    ...
    with requests.Session() as session:
        response_get = session.get(url_get, headers=API_headers, params=params, auth=HTTPBasicAuth(username, api_key))
        ...
        for event in events:
            ...
            response_del = session.delete(url_del, headers=API_headers, auth=HTTPBasicAuth(username, api_key))
    ...
    with requests.Session() as session:
        if duplicate_event:
            ...
            if server_load_target != load_target or server_time_target != time_target or server_distance_target != distance_target:
                ...
                response_put = session.put(url_put, headers=API_headers, json=put_data, auth=HTTPBasicAuth(username, api_key))
            ...
            if load_target > 0 or time_target > 0 or distance_target > 0:
                ...
                response_post = session.post(url_post, headers=API_headers, json=post_data, auth=HTTPBasicAuth(username, api_key))
