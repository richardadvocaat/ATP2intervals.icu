...
def add_load_check_description(row, previous_week_loads, previous_week_sheet_load, description):
    ...
    description += "\n---\n*made with the 5_ATP_LOAD_feedback.py script*"
    return description
...
def main():
    ...
    url_get = f"{url_base}/eventsjson".format(athlete_id=athlete_id)
    params = {"oldest": oldest_date.strftime("%Y-%m-%dT00:00:00"), "newest": newest_date.strftime("%Y-%m-%dT00:00:00")}
    ...
    for index, row in df.iterrows():
        ...
        # Get planned load from sheet for previous week
        previous_week_sheet_load = df[df['year_week'] == previous_year_week]['load'].sum()

        # Get actual load from intervals.icu for previous week
        wellness_data = get_wellness_data(athlete_id, username, api_key)
        weekly_loads = calculate_weekly_loads(wellness_data)
        previous_week_loads = weekly_loads.get(previous_year_week, {'ctlLoad': 0, 'atlLoad': 0})

        # Check if feedback note for this week already exists
        feedback_note_name = note_FEEDBACK_name_template.format(last_week=previous_week)
        note_exists = any(
            (event.get('name') == feedback_note_name and event.get('category') == 'NOTE')
            for event in events
        )

        if note_exists:
            continue  # Skip creation if feedback note exists
        ...
        if description.strip():
            create_update_or_delete_note_event(
                start_date_str, description, note_FEEDBACK_color, events,
                athlete_id, username, api_key, previous_week
            )
        time_module.sleep(parse_delay)
