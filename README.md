...
**The first Python script ([1_ATP_populate.py](https://github.com/richardadvocaat/ATP2intervals.icu/blob/main/1_ATP_populate.py))** automates the process of sending an annual training plan (ATP) with the coach's made ATP to intervals.icu so it wil make weekly targets for TSS (load), time, ore distance.
...
3. **Excel Sheets**: Place the `ATP2intervals_TLA.xlsm` and ATP_LOAD.xls in `C:\TEMP\TLA`. (TLA = e.g. RAA, so rename the sheet to ATP2intervals_RAA.xlsm).
